...
const resultProps = { scopedDocumentReviewStatuses: {}, scopedSelectedMatches: {}, onScopedDocumentReviewStatusChange: vi.fn(), onScopedSelectedMatchChange: vi.fn(), scopedPlan: null };
...
describe("ResultsView", () => {
  it("commits editable cell changes", () => {
    ...
    render(<ResultsView pages={[page]} scopedPages={[]} extractionMode="full" allPages={[{ pageNumber: 1, status: "completed", extraction: page, error: null }]} filename="x.pdf" failedCount={0} pagePreviews={[]} onReset={vi.fn()} onUpdateRow={onUpdateRow} onUpdateScopedRow={vi.fn()} onRetryPage={vi.fn()} onRetryFailed={vi.fn()} {...resultProps} />);
